...
##################################################################
#  
# Transform Core/Device Information from vn8-mcus.def to a
...
/^#/ {
	...
	if (FORMAT == "Makefile")
	{
	    ...
	    print "# Generated by    : ./gcc/config/vn8/genmultilib.awk"
	    print "# Generated from  : ./gcc/config/vn8/vn8-mcus.def"
...
##################################################################
# Run over all VN8_MCU Lines and gather Information:
# cores[]     : Enumerates the Cores (vn82, vn825, ...)
...
/^VN8_MCU/ {
	...
	# vn81 is supported for Assembler only:  It gets no multilib
	if (core == "vn81")
	    next
    ...
    # vn81 is supported for Assembler only:  Its Devices are ignored
    if (core == "vn81")
	next
    ...
    tiny_stack[name] = 0
    for (i=1; i <= n; i++)
      if (dev_attribute[i] == "VN8_SHORT_SP") {
        tiny_stack[name]  = 1
        break
      }
...
END {
    ...
    if (FORMAT == "Makefile")
    {
	# Intended Target: ./gcc/config/vn8/t-multilib
